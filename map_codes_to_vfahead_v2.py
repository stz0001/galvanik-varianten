...
Version 2: Korrekte Filterung nach Anlage über g2aktiv, g3aktiv, etc. Flags
"""
...
def get_vfahead_for_position(cursor, anlage, position_type, bmdvariante):
    ...
    if anlage == 'G2':
        anlage_condition = "vfahead_g2aktiv = 'T'"
    elif anlage == 'G3':
        anlage_condition = "vfahead_g3aktiv = 'T'"
    elif anlage == 'G4':
        anlage_condition = "vfahead_g4aktiv = 'T'"
    elif anlage == 'G5':
        anlage_condition = "vfahead_g5aktiv = 'T'"
    elif anlage in ['A60', 'A6']:
        # For A60: First try A60-specific entries, then G1 entries as fallback
        anlage_condition = """(
            vfahead_beschreibung LIKE '%%A60%%' 
            OR vfahead_beschreibung LIKE '%%A6%%'
            OR vfahead_g1aktiv = 'T'
        )"""
    else:
        return (None, None)
...
def main():
    print("Mapping Codes to VFAHEAD (Version 2)...")
    print("Using anlage-specific flags (g2aktiv, g3aktiv, etc.)")
    ...
    for anlage in ['G2', 'G3', 'G4', 'G5', 'A60']:
        ...
        if stats[0] > 0:  # Only show if there are codes for this anlage
            print(f"\n  {anlage}:")
            print(f"    Total: {stats[0]} codes")
            print(f"    VB:  {stats[1]}/{stats[0]} ({100*stats[1]//stats[0]}%)")
            print(f"    HB:  {stats[2]}/{stats[0]} ({100*stats[2]//stats[0]}%)")
            print(f"    PAS: {stats[3]}/{stats[0]} ({100*stats[3]//stats[0]}%)")
            print(f"    NB:  {stats[4]}/{stats[0]} ({100*stats[4]//stats[0]}%)")
    
    # Check for duplicates
    print("\nChecking for duplicate VFAHEAD assignments...")
    cursor.execute("""
        SELECT vfahead_inr, COUNT(*) as count
        FROM (
            SELECT vb_vfahead_inr as vfahead_inr FROM codes WHERE vb_vfahead_inr IS NOT NULL
            UNION ALL
            SELECT hb_vfahead_inr FROM codes WHERE hb_vfahead_inr IS NOT NULL
            UNION ALL
            SELECT pas_vfahead_inr FROM codes WHERE pas_vfahead_inr IS NOT NULL
            UNION ALL
            SELECT nb_vfahead_inr FROM codes WHERE nb_vfahead_inr IS NOT NULL
        ) t
        GROUP BY vfahead_inr
        HAVING COUNT(*) > 5
        ORDER BY count DESC
        LIMIT 5
    """)
    ...
    if duplicates:
        print("  Most reused VFAHEAD entries:")
        for dup in duplicates:
            cursor.execute("SELECT vfahead_beschreibung FROM vfahead WHERE vfahead_inr = %s", (dup[0],))
            desc = cursor.fetchone()[0]
            print(f"    {dup[0]}: used {dup[1]} times - {desc.strip()}")
    else:
        print("  ✓ No excessive duplication found")
